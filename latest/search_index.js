...

{
    ...
    "category": "section",
    "text": "Some of the functions we have seen require arguments. For example, when you call sin you pass a number as an argument. Some functions take more than one argument: parse takes two, a number type and a string.Inside the function, the arguments are assigned to variables called parameters. Here is a definition for a function that takes an argument:function printtwice(bruce)\n    println(bruce)\n    println(bruce)\nendThis function assigns the argument to a parameter named bruce. When the function is called, it prints the value of the parameter (whatever it is) twice.This function works with any value that can be printed.printtwice(\"Spam\")\nprinttwice(42)\nprinttwice(π)The same rules of composition that apply to built-in functions also apply to programmer-defined functions, so we can use any kind of expression as an argument for printtwice:printtwice(\"Spam \"^4)\nprinttwice(cos(π))The argument is evaluated before the function is called, so in the examples the expressions \"Spam \"^4 and cos(π) are only evaluated once.You can also use a variable as an argument:michael = \"Eric, the half a bee.\"\nprinttwice(michael)The name of the variable we pass as an argument (michael) has nothing to do with the name of the parameter (bruce). It doesn’t matter what the value was called back home (in the caller); here in printtwice, we call everybody bruce."
},

{
    ...
    "category": "section",
    "text": "When you create a variable inside a function, it is local, which means that it only exists inside the function. For example:function cattwice(part1, part2)\n    concat = part1 * part2\n    printtwice(concat)\nendThis function takes two arguments, concatenates them, and prints the result twice. Here is an example that uses it:line1 = \"Bing tiddle \"\nline2 = \"tiddle bang.\"\ncattwice(line1, line2)When cattwice terminates, the variable concat is destroyed. If we try to print it, we get an exception:println(concat)Parameters are also local. For example, outside printtwice, there is no such thing as bruce."
},
...

{
    ...
    "title": "Fruitful functions",
    "category": "section",
    "text": "Many of the Julia functions we have used, such as the math functions, produce return values. But the functions we’ve written are all void: they have an effect, like printing a value or moving a turtle, but they return nothing. In this chapter you will learn to write fruitful functions."
},
...

{
    ...
    "category": "section",
    "text": "What happens if we call fact and give it 1.5 as an argument?fact(1.5)It looks like an infinite recursion. How can that be? The function has a base case—when n == 0. But if n is not an integer, we can miss the base case and recurse forever.In the first recursive call, the value of n is 0.5. In the next, it is -0.5. From there, it gets smaller (more negative), but it will never be 0.We have two choices. We can try to generalize the factorial function to work with floating-point numbers, or we can make fact check the type of its argument. The first option is called the gamma function and it’s a little beyond the scope of this book. So we’ll go for the second.We can use the built-in operator isa to verify the type of the argument. While we’re at it, we can also make sure the argument is positive:function fact(n)\n    if !(n isa Int64)\n        println(\"Factorial is only defined for integers.\")\n        return nothing\n    elseif n < 0\n        println(\"Factorial is not defined for negative integers.\")\n        return nothing\n    elseif n == 0\n        return 1\n    else\n        return n * fact(n-1)\n    end\nendThe first base case handles nonintegers; the second handles negative integers. In both cases, the program prints an error message and returns nothing to indicate that something went wrong:println(fact(\"fred\"))\nprintln(fact(-2))If we get past both checks, we know that n is positive or zero, so we can prove that the recursion terminates.This program demonstrates a pattern sometimes called a guardian. The first two conditionals act as guardians, protecting the code that follows from values that might cause an error. The guardians make it possible to prove the correctness of the code.In Section 11.4 we will see a more flexible alternative to printing an error message: raising an exception."
},
...

{
    ...
    "title": "Strings",
    "category": "section",
    "text": "Strings are not like integers, floats, and booleans. A string is a sequence, which means it is an ordered collection of other values. In this chapter you’ll see how to access the characters that make up a string, and you’ll learn about some of the string helper functions provided by Julia."
},
...

{
    ...
    "category": "section",
    "text": "A string is a sequence of characters. This means that characters are items of the string. You can access the items one at a time with the bracket operator:fruit = \"banana\"\nletter = fruit[1]The second statement selects character number 1 from fruit and assigns it to letter.The expression in brackets is called an index. The index indicates which character in the sequence you want (hence the name).All indexing in Julia is 1-based: the first element of any integer-indexed object is found at index 1 and the last element at index end:fruit[end]As an index you can use an expression that contains variables and operators:i = 1\nfruit[i+1]\nfruit[end-1]But the value of the index has to be an integer. Otherwise you get:letter = fruit[1.5]"
},

{
    ...
    "title": "length",
    "category": "section",
    "text": "length is a built-in function that returns the number of characters in a string:fruits = \"🍌 🍎 🍐\"\nlen = length(fruits)To get the last letter of a string, you might be tempted to try something like this:last = fruits[length]But you might not get what you expect.Strings are encoded using the UTF-8 encoding. UTF-8 is a variable-width encoding, meaning that not all characters are encoded in the same number of bytes.The function sizeof gives the number of bytes in a string:sizeof(\"🍌\")Because an emoji is encoded in 4 bytes and string indexing is byte based, the 5th element of fruits is a SPACE.This means also that not every byte index into a UTF-8 string is necessarily a valid index for a character. If you index into a string at such an invalid byte index, an error is thrown:fruits[2]In the case of fruits, the character 🍌 is a four-byte character, so the indices 2, 3 and 4 are invalid and the next character\'s index is 5; this next valid index can be computed by nextind(fruit, 1), and the next index after that by nextind(fruit, 5) and so on."
},
...

{
    ...
    "title": "String slices",
    "category": "section",
    "text": "A segment of a string is called a slice. Selecting a slice is similar to selecting a character:str = \"Julius Caesar\"\nstr[1:6]The operator [n:m] returns the part of the string from the “n-eth” byte to the “m-eth” byte. So the same caution is needed as for simple indexing.The end keyword can be used to indicate the last byte of the string:str[8:end]If the first index is greater than the second the result is an empty string, represented by two quotation marks:str[8:7]An empty string contains no characters and has length 0, but other than that, it is the same as any other string.Continuing this example, what do you think str[:] means? Try it and see."
},

{
    ...
    "category": "section",
    "text": "It is tempting to use the [] operator on the left side of an assignment, with the intention of changing a character in a string. For example:greeting = \"Hello, world!\"\ngreeting[0] = \'J\'The reason for the error is that strings are immutable, which means you can’t change an existing string. The best you can do is create a new string that is a variation on the original:new_greeting = \"J\" * greeting[2:end]This example concatenates a new first letter onto a slice of greeting. It has no effect on the original string."
},

{
    ...
    "category": "section",
    "text": "Constructing strings using concatenation can become a bit cumbersome, however. To reduce the need for these verbose calls to string or repeated multiplications, Julia allows string interpolation using $:greet = \"Hello\"\nwhom = \"World\"\n\"$greet, $whom!\"This is more readable and convenient than string concatenation: greet * \", \" * whom * \"!\"The shortest complete expression after the $ is taken as the expression whose value is to be interpolated into the string. Thus, you can interpolate any expression into a string using parentheses:\"1 + 2 = $(1 + 2)\""
},
...

{
    ...
    "category": "section",
    "text": "Julia provides functions that perform a variety of useful operations on strings. For example, the function uppercase takes a string and returns a new string with all uppercase letters.uppercase(\"Hello, World!\")As it turns out, there is a function named search that is remarkably similar to the function find we wrote:search(\"banana\", \'a\')Actually, the search function is more general than our function; it can find substrings, not just characters:search(\"banana\", \"na\")By default, search starts at the beginning of the string, but it can take a third argument, the index where it should start:search(\"banana\", \"na\", 4)This is an example of an optional argument."
},

{
    ...
    "title": "The ∈ operator",
    "category": "section",
    "text": "The keyword ∈ (\\in TAB) is a boolean operator that takes a character and a string and returns true if the first appears as in the second:\'a\' ∈ \"banana\"    # \'a\' in \"banana\"For example, the following function prints all the letters from word1 that also appear in word2:function inboth(word1, word2)\n    for letter in word1\n        if letter ∈ word2\n            println(letter)\n        end\n    end\nendWith well-chosen variable names, Julia sometimes reads like English. You could read this loop, “for (each) letter in (the first) word, if (the) letter is an element of (the second) word, print (the) letter.”Here’s what you get if you compare \"apples\" and \"oranges\":using ThinkJuliainboth(\"apples\", \"oranges\")"
},
...

{
    ...
    "category": "section",
    "text": "For the exercises in this chapter we need a list of English words. There are lots of word lists available on the Web, but the one most suitable for our purpose is one of the word lists collected and contributed to the public domain by Grady Ward as part of the Moby lexicon project (see http://wikipedia.org/wiki/Moby_Project). It is a list of 113809 official crosswords; that is, words that are considered valid in crossword puzzles and other word games. In the Moby collection, the filename is 113809of.fic; you can download a copy, with the simpler name words.txt, from https://github.com/BenLauwens/ThinkJulia.jl/data/words.txt.This file is in plain text, so you can open it with a text editor, but you can also read it from Julia. The built-in function open takes the name of the file as a parameter and returns a file stream you can use to read the file.julia> fin = open(\"words.txt\")\nIOStream(<file words.txt>)fin is a common name for a file stream used for input. Julia provides several function for reading, including readline, which reads characters from the file until it gets to a NEWLINE and returns the result as a string:fin = open(\"../../data/words.txt\")readline(fin)The first word in this particular list is “aa”, which is a kind of lava. The file stream keeps track of where it is in the file, so if you call readline again, you get the next word:readline(fin)The next word is “aah”, which is a perfectly legitimate word, so stop looking at me like that.You can also use a file as part of a for loop. This program reads words.txt and prints each word, one per line:for line in eachline(\"words.txt\")\n    println(line)\nend"
},
...

{
    ...
    "category": "section",
    "text": "The syntax for accessing the elements of an array is the same as for accessing the characters of a string—the bracket operator. The expression inside the brackets specifies the index. Remember that the indices start at 1:cheeses[1]Unlike strings, arrays are mutable. When the bracket operator appears on the left side of an assignment, it identifies the element of the array that will be assigned:numbers = [42, 123];\nnumbers[2] = 5;\nprint(numbers)The second element of numbers, which used to be 123, is now 5.Figure 10.1 shows the state diagrams for cheeses, numbers and empty.using ThinkJulia\nfig10_1()<figure>\n  <img src=\"fig101.svg\" alt=\"State diagrams.\">\n  <figcaption>Figure 10.1. State diagrams.</figcaption>\n</figure>\\begin{figure}\n\\centering\n\\includegraphics{fig101}\n\\caption{State diagrams.}\n\\label{fig101}\n\\end{figure}Arrays are represented by boxes and the elements of the array inside. cheeses refers to an array with three elements indexed 1, 2 and 3. numbers contains two elements; the diagram shows that the value of the second element has been reassigned from 123 to 5. empty refers to an array with no elements.Array indices work the same way as string indices:Any integer expression can be used as an index.\nIf you try to read or write an element that does not exist, you get a BoundsError.\nThe keyword end points to the last index of the array.The ∈ operator also works on arrays:\"Edam\" ∈ cheeses\n\"Brie\" in cheeses"
},
...

{
    ...
    "title": "Array slices",
    "category": "section",
    "text": "The slice operator also works on arrays:t = [\'a\', \'b\', \'c\', \'d\', \'e\', \'f\'];\nprint(t[1:3])\nprint(t[3:end])The slice operator [:], makes a copy of the whole array:print(t[:])Since arrays are mutable, it is often useful to make a copy before performing operations that modify arrays.A slice operator on the left side of an assignment can update multiple elements:t[2:3] = [\'x\', \'y\'];\nprint(t)"
},

{
    ...
    "category": "section",
    "text": "Julia provides functions that operate on arrays. For example, push! adds a new element to the end of an array:t = [\'a\', \'b\', \'c\'];\npush!(t, \'d\');\nprint(t)append! add the elements of the second array to the end of the first:t1 = [\'a\', \'b\', \'c\'];\nt2 = [\'d\', \'e\'];\nappend!(t1, t2);\nprint(t1)This example leaves t2 unmodified.sort! arranges the elements of the array from low to high:t = [\'d\', \'c\', \'e\', \'b\', \'a\'];\nsort!(t);\nprint(t)sort returns a copy of the elements of the array in order:t1 = [\'d\', \'c\', \'e\', \'b\', \'a\'];\nt2 = sort(t1);\nprint(t1)\nprint(t2)As a style convention in Julia, ! is appended to names of functions that modify their arguments."
},
...

{
    ...
    "title": "Dot syntax",
    "category": "section",
    "text": "For every binary operator like ^, there is a corresponding dot operator .^ that is automatically defined to perform ^ element-by-element on arrays. For example, [1, 2, 3] ^ 3 is not defined, but [1, 2, 3] .^ 3 is defined as computing the elementwise result [1^3, 2^3, 3^3]:print([1, 2, 3] .^ 3)Any Julia function f can be applied elementwise to any array with the dot syntax. For example to capitalize an array of strings, we don\'t need a loop:t = uppercase.([\"abc\", \"def\", \"ghi\"]);\nprint(t)"
},

{
    ...
    "category": "section",
    "text": "There are several ways to delete elements from an array. If you know the index of the element you want, you can use splice!:t = [\'a\', \'b\', \'c\'];\nsplice!(t, 2)\nprint(t)splice! modifies the array and returns the element that was removed.pop! deletes and returns the last element:t = [\'a\', \'b\', \'c\'];\npop!(t)\nprintln(t)shift! deletes and returns the first element:t = [\'a\', \'b\', \'c\'];\nshift!(t)\nprintln(t)The functions unshift! and push! insert an element at the beginning, respectively at the end of the array. If you don’t need the removed value, you can use the function deleteat!:t = [\'a\', \'b\', \'c\'];\nprint(deleteat!(t, 2))The function insert! inserts an element at a given index:t = [\'a\', \'b\', \'c\'];\nprint(insert!(t, 2, \'x\'))"
},

{
    ...
    "category": "section",
    "text": "A string is a sequence of characters and an array is a sequence of values, but an array of characters is not the same as a string. To convert from a string to an array of characters, you can use the function collect:t = collect(\"spam\");\nprint(t)The collect function breaks a string or another sequence into individual elements. If you want to break a string into words, you can use the split function:t = split(\"pining for the fjords\");\nprint(t)An optional argument called a delimiter specifies which characters to use as word boundaries. The following example uses a hyphen as a delimiter:t = split(\"spam-spam-spam\", \'-\');\nprint(t)join is the inverse of split. It takes an array of strings and concatenates the elements:t = [\"pining\", \"for\", \"the\", \"fjords\"];\ns = join(t, \' \')In this case the delimiter is a space character. To concatenate strings without spaces, you don\'t specify a delimiter."
},
...

{
    ...
    "title": "Aliasing",
    "category": "section",
    "text": "If a refers to an object and you assign b = a, then both variables refer to the same object:a = [1, 2, 3];\nb = a;\nb ≡ aThe state diagram looks like Figure 10.4.using ThinkJulia\nfig10_4()<figure>\n  <img src=\"fig104.svg\" alt=\"State diagram.\">\n  <figcaption>Figure 10.4. State diagram.</figcaption>\n</figure>\\begin{figure}\n\\centering\n\\includegraphics{fig104}\n\\caption{State diagram.}\n\\label{fig104}\n\\end{figure}The association of a variable with an object is called a reference. In this example, there are two references to the same object.An object with more than one reference has more than one name, so we say that the object is aliased.If the aliased object is mutable, changes made with one alias affect the other:b[1] = 42;\nprint(a)Although this behavior can be useful, it is error-prone. In general, it is safer to avoid aliasing when you are working with mutable objects.For immutable objects like strings, aliasing is not as much of a problem. In this example:a = \"banana\"\nb = \"banana\"It almost never makes a difference whether a and b refer to the same string or not."
},
