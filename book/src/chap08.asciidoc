...
The function +firstindex+ returns the first valid byte index. The keyword +global+ before +index+ indicates that we want to modify a variable defined outside the loop (see <<global_variables>>).
