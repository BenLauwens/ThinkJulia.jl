...
In Julia you have the ability to write code that can operate on different types. This is called polymorphism. Julia's type system is dynamic but inherits some features of static type system. A method can be efficiently dispatched on the number and the type of its arguments.
