...
As you can see, type declaration can and should be added for performance reasons to the fields in a struct definition.
...
An inner constructor method is always declared inside the block of a type declaration and it has access to a special function called +new+ that creates objects of the newly declared type.
