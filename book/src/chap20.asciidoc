...
[WARNING]
====
A function in Julia is compiled the first time it is executed. So to compare two algorithms, they have to be implemented as functions to get compiled and the first time they are called has to be excluded from the performance measure.
====
...
Julia specifies some informal interfaces to define behaviors. When you implement these methods for a type, objects of that type can be used in methods to build upon these behaviors.
(((interface)))
...
(((@noinline)))((("macro", "Base", "@noinline", see="@noinline")))
