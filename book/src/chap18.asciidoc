...
Variables like +suit_names+ and +rank_names+, which are defined outside of a type definition or of any method, are called _global variables_. The +const+ declaration means that the variable can only be assigned once. This solves the performance problem of global variables.
