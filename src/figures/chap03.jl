...
function fig03_1()
  p = TikzPicture(L"""
  \node[anchor=east] at(-3.75,0){\tt \_\_main\_\_};
  \node[draw, fill=lightgray, minimum width=6.5cm, minimum height=1cm]{};
  ...
  \draw[-latex] (l2) -- (l2v);
  \node[anchor=east] at(-3.75,-1.5){\tt cattwice};
  \node[draw, fill=lightgray, minimum width=6.5cm, minimum height=1.5cm] at(0,-1.5){};
  ...
  \draw[-latex] (cc) -- (ccv);
  \node[anchor=east] at(-3.75,-2.75){\tt printtwice};
  \node[draw, fill=lightgray, minimum width=6.5cm, minimum height=0.5cm] at(0,-2.75){};
